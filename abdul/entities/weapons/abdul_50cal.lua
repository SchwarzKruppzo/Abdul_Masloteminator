...
SWEP.WorldModel   = "models/props_marines/50cal.mdl"
...
function SWEP:Think()
	...
	if SERVER then
		...
		local trRight = util.TraceLine({
			start = trMain.HitPos,
			endpos = trMain.HitPos - right * 15,
			filter = self.Owner
		})
		local trLeft = util.TraceLine({
			start = trMain.HitPos,
			endpos = trMain.HitPos + right * 5,
			filter = self.Owner
		})
		local trForward = util.TraceLine({
			start = trMain.HitPos + up * 5,
			endpos = trMain.HitPos + up * 5 + forward * 5,
			filter = self.Owner
		})
		...
		if trMain.Hit and !trRight.Hit and !trLeft.Hit and !trForward.Hit and !trBack.Hit and !FindMines( pos ) and self.Owner:IsPlantLimit( "50cal" ) then
...
function SWEP:DrawHUD()
	if CLIENT then
		...
		if !IsValid(self.GhostEntity) then
			...
			self.GhostEntity:SetModel("models/props_marines/50cal.mdl")
...
function SWEP:PrimaryAttack()
	...
	if SERVER then
		...
		local curangle = Angle(0,angle.y,0)
		if self:GetCanPlant() then
			local ent = ents.Create ("ent_50cal")
			...
			self.Owner:AddPlantLimit("50cal")
		end
	end
