...
if CLIENT then
	...
	function ENT:Initialize()
		self.OriginPos = self:GetPos() + self:GetUp()*15
		self.Spin = 0
	...
	function ENT:DrawTranslucent()
		...
		if ( ViewDot >= 0 ) then
			...
			render.DrawSprite( self:GetPos()+ self:GetUp() *5, Size,Size,Color(50,50,50,Alpha))
			render.DrawSprite( self:GetPos()+ self:GetUp() *5, Size,Size,Color(255,180,0,Alpha))
			render.DrawSprite( self:GetPos()+ self:GetUp() *5, Size,Size,Color(255,180,0,Alpha))
			render.DrawSprite( self:GetPos()+ self:GetUp() *5, Size,Size,Color(255,180,0,Alpha))
		end
	...
	function ENT:Think()
		...
		for k,v in pairs (player.GetAll()) do
			...
			if v:GetPos():Distance(self:GetPos()) < 50 and self.HasArmor == true and v:Alive() and v:Armor() < 255 then
				self.HasArmor = false
				self:SetNoDraw(true)
				self.RespawnTime = CurTime() + GetConVar("abdul_armoryellow_restime"):GetInt()
				v:EmitSound("items/battery_pickup.wav",40,100)
				v:SetArmor(math.Clamp(v:Armor() + 80,0,255))
				hook.Call("PlayerPickupPowerup",GAMEMODE,v,"armoryellow")
			end
		end
	end
