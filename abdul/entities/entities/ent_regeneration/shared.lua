...
if CLIENT then
	...
	function ENT:DrawTranslucent()
		...
		local ViewDot = 1
		local LightPos = self:GetPos()+ self:GetUp() * 16
		if ( ViewDot >= 0 ) then
			...
			render.DrawSprite( self:GetPos()+ self:GetUp() * 16, Size,Size,Color(0,255,0,Alpha))
			render.DrawSprite( self:GetPos()+ self:GetUp() * 16, Size,Size,Color(0,255,0,Alpha))
		end
		self:SetupBones()
		self:DrawModel()
	...
	function ENT:SpawnFunction(ply, tr)
		...
		local ent = ents.Create("ent_regeneration")
		ent:SetPos(SpawnPos)
	...
	function ENT:Initialize()
		self:SetModel("models/w_regeneration.mdl")
		...
		self.HasRegen = PU_CanSpawnPowerUp("Regeneration")
		self.HasRegenR = true
		self.DefaultTime = 40
		self.DieTime = 0
		self.NextPickup = {}
	...
	function ENT:Think()
		if !self.NoRespawn then
			if PU_CanSpawnPowerUp("Regeneration") then
			...
			elseif !PU_CanSpawnPowerUp("Regeneration") then
			...
			for k,v in pairs (player.GetAll()) do
				...
				if v:GetPos():Distance(self:GetPos()) < 45 and self.HasRegen == true and self.HasRegenR and v:Alive() then
					...
					PU_SetPowerUp( "Regeneration", v, self.DefaultTime )
					hook.Call("PlayerPickupPowerup",GAMEMODE,v,"regeneration")
				end
			end
		else
			local tracedata = {}
